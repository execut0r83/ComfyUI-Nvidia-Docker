...
if [ $do_change == "True" ]; then
  # Make a "comfytoo" user if it doesn't already exist
...
if [ ! -z "$WANTED_GID" -a "$WANTED_GID" != "$new_gid" ]; then
    echo "Warning: Wrong GID ($new_gid), but proceeding."
    # Не выходить, а просто продолжить
fi

if [ ! -z "$WANTED_UID" -a "$WANTED_UID" != "$new_uid" ]; then
    echo "Warning: Wrong UID ($new_uid), but proceeding."
    # Не выходить, а просто продолжить
fi
